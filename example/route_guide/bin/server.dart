...
class SecurityContextChannelCredentials extends grpc.ChannelCredentials {
  final SecurityContext _securityContext;

  SecurityContextChannelCredentials(SecurityContext securityContext,
      {String? authority, grpc.BadCertificateHandler? onBadCertificate})
  ...
  static SecurityContext baseSecurityContext() {
    return grpc.createSecurityContext(false);
  }
}

class SecurityContextServerCredentials extends grpc.ServerTlsCredentials {
  final SecurityContext _securityContext;
  ...
  SecurityContext get securityContext => _securityContext;
  static SecurityContext baseSecurityContext() {
    return grpc.createSecurityContext(true);
  }
}
...
Future<void> main(List<String> args) async {
  ...
  final bytesKey = await File('lib/assets/data/private.key').readAsBytes();
  final privateKey = Uint8List.view(bytesKey.buffer);
  serverContext.usePrivateKeyBytes(privateKey);
  final grpc.ServerCredentials serverCredentials =
